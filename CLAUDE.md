...
- People responsible for a record are many (`App\Core\Concerns\HasAssignees`, table `record_assignees`); save them with `App\Support\Assignments::sync()` so the people added are notified. Forms post `assignees[]`.
